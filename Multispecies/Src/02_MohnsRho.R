...
#### Yellowtail ####
modtemp <- arrange(yt_lfo10,RMSE)[1,]%>%#select best model
...
ggplot()+ #you could add observations onto this to show contrast
  facet_wrap(~var)+
  geom_point(data=plot.dat,alpha=0.6, aes( label=year,y=Y_rec, x=value, col=year))+
  geom_text(data=plot.dat, aes( label=year,y=Y_rec, x=value,col=year),alpha=0.6,nudge_y = 0.05)+
  xlim(c(-3,3))+
...
ts_smooths <- data.frame()
mohns <- data.frame()
results <- FunctionalRelationships(sb_dat, 15)
...
plot.dat<-sb_dat%>%select(year,Y_rec,unique(results[[1]]$var))%>%
  pivot_longer(cols=c(unique(results[[1]]$var)))%>%
  rename(var=name)
...
ts_smooths <- data.frame()
mohns <- data.frame()
results <- FunctionalRelationships(ps_dat, 15)
mohns15<- data.frame(results[[2]])%>%
  group_by(row.names.xyy.)%>%
  summarize(mohns15=sum(mohns)/length(unique(termyr)))

plot.dat<-ps_dat%>%select(year,Y_rec,unique(results[[1]]$var))%>%
  pivot_longer(cols=c(unique(results[[1]]$var)))%>%
  rename(var=name)
