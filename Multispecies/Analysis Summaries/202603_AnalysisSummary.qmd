...
hk<-read.csv("Output/Data/hk_table.csv")
loohk<-hk%>%
  select(variables,Hit,  dev.ex, AIC, RMSE_loo,RMSE5, RMSE10)%>%
  arrange(RMSE_loo)%>%
  mutate(SelectionCriteria="LOO")%>%
...
![Mean Mohns rho by terminal year for single predictor models showing depicting mean values across oceanographic predictors by species with a loess smooth by species](Output/Figures/mohnstrend.png){#fig-mohnstrend}
...
## Investigating the 2012 break point as a potential driver of stability/observed changes

### Motivation

Based on the findings in 1 - 3 there appears to be 1) a decline in model stability that is delineated by a 2012 - 2015 breakepoint @fig-mohnstrend and 2) a change in the relative importance of different drivers @fig-rw with a breakpoint for most species in the early 2010s. 

### Methods

To dig into this a bit, I divided oceanographic conditions into two groups 1) stable variables that have a Mohn's rho of 0 and 2) unstable variables that have a Mohn's rho of 0.1 (NOTE: I played around with how I divided these quite a bit and the results were not sensitive to it).

I fit a bayesian DFA to the two groups of ocenographic condtions to see if there were differences in the underlying patterns of variability. I tested up to three trends and for both groups of variables the best supported models only had 1.

### Findings

![Bayesian dynamic factor analysis for "stable" and "unstable" variables](Output/Figures/dfas.png){#fig-dfa}

We do not observe differences between the two groups of variables and there are strong loadings across most time series across a similar latent trend. One very notable observation about this trend is that for most of the 1994 - 2018 time period the latent trend is at or below the long-term mean. However, two notable exceptions exist 1) 1997/1998 and 2) 2014 onward. 1997/1998 was the second largest El Nino event on record that had huge consequences for dynamic height, water mass characteristics, and a temporary reversal of net alongshore transport (@Lynn2002) which also had substantial impact on the ecosystem (@Ronald1998). The 2014 onward period doesn't need much explaining (marine heatwaves, pan basin warming).

## Preliminary Conclusion

Collectively, it seems like the inclusion(exclusion) of the 2010 - 2018 time period in the training of the model greatly impacts variable importance and model stability for recruitment (@fig-mohnstrend). This in turn, results in incosistent model selection across selection criteria, as they weigh the 2009 - 2018 time period differently across criteria. We also find that the stability of these models and the differences in prediction capacity does vary by species, with yellowtail showing more stability and consistent variable importance and also greater consistency in model selection across the suite of selection criteria we used. 

**Question: to dig into this a little deeper we could fit the models from 1994 - 2010 and see if those models are more stable? and generate the same plots of @fig-mohnstrend and @fig-mohns**
